...
class PlayState extends MusicBeatState
{
	...
	var endMusicsak:FlxSound;
	var endMusiczer:FlxSound;
	...
	// If you found this line, no you didn't. - TK
	...
	/*public function youshouldwalkNOW()
	{
		if(walkin)crowd.x += 5;
	}*/
	...
	//	snowfgstrong2.setGraphicSize(Std.int((snowfgstrong2.width * 1.2) / defaultCamZoom));
		...
		if (isStoryMode && SONG.player1 == 'bf-cold')
		{
			...
			/*endMusic2 = new FlxSound().loadEmbedded(Paths.music('end2', 'shared'), false, true); // week 2
			FlxG.sound.list.add(endMusic2);
			/*endMusicsak = new FlxSound().loadEmbedded(Paths.music('endsak', 'shared'), false, true); // sakuroma
			FlxG.sound.list.add(endMusicsak);
			endMusiczer = new FlxSound().loadEmbedded(Paths.music('endzer', 'shared'), false, true); // zerktro
			FlxG.sound.list.add(endMusiczer);*/
		}
		/*else if (!isStoryMode && SONG.song == 'Cold-Hearted')
		{
			endMusicsak = new FlxSound().loadEmbedded(Paths.music('endsak', 'shared'), false, true); // sakuroma
			FlxG.sound.list.add(endMusicsak);
			endLoop = new FlxSound().loadEmbedded(Paths.music('endLoop', 'shared'), true, true);
		...
		else if (!isStoryMode && SONG.song == 'Ectospasm')
		{
			endMusiczer = new FlxSound().loadEmbedded(Paths.music('endzer', 'shared'), false, true); // zerktro
			FlxG.sound.list.add(endMusiczer);
			endLoop = new FlxSound().loadEmbedded(Paths.music('endLoop', 'shared'), true, true);
			...
			switch (stageCheck)
			{
				case 'city' | 'stage':
					...
					case 'bridge':
						...
						bgcold.scrollFactor.set(0.9, 0.9);
						bgcold.active = false;
						add(bgcold);
						bgcold.alpha = 0;
						...
						bridgecold.scrollFactor.set(0.9, 0.9);
						...
						fgcold.scrollFactor.set(0.9, 0.9);
						fgcold.active = false;
						add(fgcold);
						fgcold.alpha = 0;
				...
				isPixelStage = PlayState.curStage.endsWith('-pixel');
			}
		...
		switch (curStage)
		{
			case 'city':
				...
				dad.x -= 345;
				dad.y += 140;
				camFollow.x == dad.getGraphicMidpoint().x -  30;
				camFollow.y == dad.getGraphicMidpoint().y + 0;
				camFollow.x == boyfriend.getGraphicMidpoint().x +  10;
				camFollow.y == boyfriend.getGraphicMidpoint().y + 250;
			...
			case 'bridge':
				...
				dad.x -= 350;
				dad.y -= 170;
				camFollow.x == dad.getGraphicMidpoint().x +  10;
				camFollow.y == dad.getGraphicMidpoint().y + 250; trace('works');
				camFollow.x == boyfriend.getGraphicMidpoint().x +  10;
				camFollow.y == boyfriend.getGraphicMidpoint().y + 250; trace('works but bf edition');
		...
		if (!PlayStateChangeables.Optimize)
		{
			if (SONG.song == 'Sub-Zero' || SONG.song == 'Frostbite' || SONG.song == 'Cryogenic')
			{
				bgDarken = new FlxSprite(-1000, -400).makeGraphic(3500, 2550, FlxColor.BLACK);
				bgDarken.scale.set(1.30, 1.30);
				if (SONG.song == 'Sub-Zero')
					bgDarken.alpha = 0;
				else if (SONG.song == 'Cryogenic')
					bgDarken.alpha = 0;
				else
					bgDarken.alpha = 0.5;
		...
		//trace('' + HealthIcon.startColor(iconP2));
		...
		if (curSong == 'Frostbite' && dad.curCharacter == 'ace')
	...
	public function generateSong(dataPath:String):Void
	{
		...
		// Randomize ice notes
		if (FlxG.save.data.specialMechanics && (curSong == 'Noreaster' || curSong == "Sub-Zero" || curSong == 'Frostbite' || curSong == 'Cold-Front' || curSong == 'Cryogenic' || curSong == 'North' || curSong == 'Cold-Hearted' || curSong == 'Ectospasm' ))
		{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var breakAnim:FlxSprite = new FlxSprite(-10, strumLine.y);
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText + " " + curSong.replace('-', ' ') + " (" + storyDifficultyText + ") "
					+ Ratings.GenerateLetterRank(accuracy),
				...
				DiscordClient.changePresence(detailsText,
					curSong.replace('-', ' ') + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), iconRPC);
			#end
		}
		...
		//update loop stuff lul
	{
		...
		if(walkinRight)crowd.x += 2;
		if(walkinLeft)crowd.x -= 2;


		if(!newYorker && curStage != 'city' &&  curStage != 'bridgecrime')
			{
		...
		else if ((controls.PAUSE || controls.ACCEPT) && endingSong && endScreen)
		{
			...
			/*if (endMusic2.playing)
				{
					endMusic2.fadeOut(1, 0, function(flx:FlxTween)
					{
						endMusic2.stop();
					});
					endMusic2.onComplete = null;
				}
				else
					endLoop.fadeOut(1, 0, function(flx:FlxTween)
					{
			...
			if (endMusicsak.playing)
				{
					endMusicsak.fadeOut(1, 0, function(flx:FlxTween)
					{
						endMusicsak.stop();
					});
					endMusicsak.onComplete = null;
				}
				else
					endLoop.fadeOut(1, 0, function(flx:FlxTween)
					{
			...
			if (endMusiczer.playing)
				{
					endMusiczer.fadeOut(1, 0, function(flx:FlxTween)
					{
						endMusiczer.stop();
					});
					endMusiczer.onComplete = null;
				}
				else
					endLoop.fadeOut(1, 0, function(flx:FlxTween)
					{
			...
			if (!FreeplayState.unlockedSongs[9] && !PlayStateChangeables.botPlay)
			{
				FreeplayState.unlockedSongs[3] = true; trace('omg it works it unlocks the song!!!');
				FreeplayState.unlockedSongs[7] = true; trace('omg it works it unlocks the song!!!');
				FreeplayState.unlockedSongs[8] = true; trace('omg it works it unlocks the song!!!');
				FreeplayState.unlockedSongs[9] = true; trace('omg it works it unlocks the song!!!');
				FlxG.save.data.unlockedSongs = FreeplayState.unlockedSongs;
				StoryMenuState.unlockedSongs.push('Frostbite'); trace('omg it works it unlocks the song!!!');
				StoryMenuState.unlockedSongs.push('north'); trace('omg it works it unlocks the song!!!');
				StoryMenuState.unlockedSongs.push('cold-hearted'); trace('omg it works it unlocks the song!!!');
				StoryMenuState.unlockedSongs.push('ectospasm'); trace('omg it works it unlocks the song!!!');
				FlxG.save.flush();
			}
			...
			if (camFollow.x != dad.getMidpoint().x + 500 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
				{
					...
					if (FlxG.save.data.camzoom && curSong == 'Frostbite' && curBeat >= 448 && curBeat < 508)
						camFollow.setPosition(dad.getMidpoint().x + 150 + offsetX, dad.getMidpoint().y - 375 + offsetY);
					else if (curStage == 'bridge')
						{
							camFollow.setPosition(dad.getMidpoint().x + 150 + offsetX, dad.getMidpoint().y - 450 + offsetY);
						}
				...
				if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 450)
				{
					...
					if (FlxG.save.data.camzoom && curSong == 'Frostbite' && curBeat >= 448 && curBeat < 508)
						...
						{
							camFollow.setPosition(boyfriend.getMidpoint().x - 100 + offsetX, boyfriend.getMidpoint().y - 450 + offsetY);
						}
						...
						camFollow.setPosition(boyfriend.getMidpoint().x
							...
							- 250
							+ offsetY
							+ (boyfriend.curCharacter == 'bf-retro' ? -15 : 0));
		...
		if (FlxG.save.data.camzoom && camZooming && !endingSong)
		{
			if (curSong == 'Frostbite' && curBeat >= 448 && curBeat < 508)
				FlxG.camera.zoom = FlxMath.lerp(1.00, FlxG.camera.zoom, 0.50);
			else
		...
		if (health <= 0 && !cannotDie)
		{
			...
			DiscordClient.changePresence("GAME OVER -- " + curSong.replace('-', ' ') + " (" + storyDifficultyText + ") "
				+ Ratings.GenerateLetterRank(accuracy),
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				var strumGroup:FlxTypedGroup<FlxSprite> = daNote.mustPress ? playerStrums : strumLineNotes;
	
				var strum:FlxSprite = strumGroup.members[daNote.noteData];
				...
				var swagWidth:Float = Note.swagWidth / 2;
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0 && StoryMenuState.curWeek == 0) 
			{
				trace('cry!!');
				if (storyChar == 0)
				...
				if (boyfriend.curCharacter == 'bf-cold')
				{
					...
					fade.alpha = 0;
					fade.cameras = [camHUD];
					endImage = new FlxSprite(0, 0).loadGraphic(Paths.image('End', 'week-ace')); 	trace('cry3!!');
					endImage.alpha = 0;
				...
				else
				{
					doof.finishThing = function()
					{
						...
						{
							FreeplayState.unlockedSongs[3] = true; trace('omg it works it unlocks the song!!!2');
							FlxG.save.data.unlockedSongs = FreeplayState.unlockedSongs;
							StoryMenuState.unlockedSongs.push('Frostbite');  trace('omg it works it unlocks the song!!!2');
							FlxG.save.flush();
						}
			...
			else if (storyPlaylist.length <= 0 && StoryMenuState.curWeek == 1)
			{
				...
				if (boyfriend.curCharacter == 'bf-cold')
				{
					...
					fade.alpha = 0;
					fade.cameras = [camHUD];
					endImage = new FlxSprite(0, 0).loadGraphic(Paths.image('End2', 'week-ace'));
					endImage.alpha = 0;
					endImage.cameras = [camHUD];
					...
					doof.finishThing = function()
					{
						...
						FlxTween.tween(fade, {alpha: 1}, 1, {
							onComplete: function(flx:FlxTween)
							{
								...
								add(endImage);
								endMusic = new FlxSound().loadEmbedded(Paths.music('end2', 'shared'), false, true);
								endMusic.play(true);
								endMusic.onComplete = function()
								{
									endMusic.onComplete = null;
				...
				else
				{
					doof.finishThing = function()
					{
						...
						if (!FreeplayState.unlockedSongs[9] && !PlayStateChangeables.botPlay)
						{
							FreeplayState.unlockedSongs[7] = true;  trace('omg it works it unlocks the song!!!2');
							FreeplayState.unlockedSongs[8] = true;  trace('omg it works it unlocks the song!!!2');
							FreeplayState.unlockedSongs[9] = true;  trace('omg it works it unlocks the song!!!2');
							FlxG.save.data.unlockedSongs = FreeplayState.unlockedSongs; 
							StoryMenuState.unlockedSongs.push('north');  trace('omg it works it unlocks the song!!!2');
							StoryMenuState.unlockedSongs.push('cold-hearted');  trace('omg it works it unlocks the song!!!2');
							StoryMenuState.unlockedSongs.push('ectospasm');  trace('omg it works it unlocks the song!!!2');
							FlxG.save.flush();
						}
						/*if (!StoryMenuState.characterUnlocked[1])
						{
		...
		else
		{
			...
			if ( !isStoryMode && SONG.song == 'Cold-Hearted')
			{
				// dialogue
				doof = new DialogueBox(dialogue);
				doof.scrollFactor.set();
				...
				fade.alpha = 0;
				fade.cameras = [camHUD];
				endImage = new FlxSprite(0, 0).loadGraphic(Paths.image('SakuGetsCockblocked', 'week-ace'));
				endImage.alpha = 0;
				endImage.cameras = [camHUD];
				...
				doof.finishThing = function()
                {
                    ...
                    FlxTween.tween(fade, {alpha: 1}, 1, {
                        onComplete: function(flx:FlxTween)
                        {
                            ...
                            add(endImage);
							endMusic = new FlxSound().loadEmbedded(Paths.music('endsak', 'shared'), false, true);
                            endMusic.play(true);
                            endMusic.onComplete = function()
                            {
                                endMusic.onComplete = null;
			...
			else if (!isStoryMode && SONG.song == 'Ectospasm')
			{
				...
				fade.alpha = 0;
				fade.cameras = [camHUD];
				endImage = new FlxSprite(0, 0).loadGraphic(Paths.image('EctospasmEnd', 'week-ace'));
				endImage.alpha = 0;
				endImage.cameras = [camHUD];
				...
				doof.finishThing = function()
                {
                    ...
                    FlxTween.tween(fade, {alpha: 1}, 1, {
                        onComplete: function(flx:FlxTween)
                        {
                            ...
                            add(endImage);
							endMusic = new FlxSound().loadEmbedded(Paths.music('endzer', 'shared'), false, true);
                            endMusic.play(true);
                            endMusic.onComplete = function()
                            {
                                endMusic.onComplete = null;
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray.contains(true) || PlayStateChangeables.botPlay))
				{
					if (boyfriend.animation.curAnim.name.startsWith('sing')
						...
						&& boyfriend.animation.curAnim.curFrame >= 10)
						boyfriend.playAnim('idle');
				}
		...
		if (!note.wasGoodHit)
		{
			...
			playerStrums.forEach(function(spr:FlxSprite)
			{
				...
				if (note.noteData == spr.ID && note.isFreezeNote)
				{
					...
					breakAnims.members[spr.ID].centerOffsets();
					breakAnims.members[spr.ID].centerOrigin();
	...
	override function stepHit()
	{
		...
		if (SONG.song == 'Cryogenic' && curBeat == 151)
			{
	...
	override function beatHit()
	{
		...
		if (!PlayStateChangeables.Optimize)
		{
								...
								new FlxTimer().start(0.2, function(tmr:FlxTimer)
									{
										if (!paused)
										{
											snowfgweak.alpha += 0.1; 
											snowfgweak2.alpha += 0.1; 
										}
			
										if (snowfgweak2.alpha < 1)
											tmr.reset(0.1);
								...
								new FlxTimer().start(0.2, function(tmr:FlxTimer)
									{
										...
										{
											snowfgmid.alpha += 0.1; 
											snowfgmid2.alpha += 0.1; 
										}
			
										if (snowfgmid2.alpha < 1)
											tmr.reset(0.1);
								...
								new FlxTimer().start(0.2, function(tmr:FlxTimer)
									{
										if (!paused)
										{
											snowfgstrong.alpha += 0.1; 
											snowfgstrong2.alpha += 0.1; 

										}
			
										if (snowfgstrong.alpha < 0.5)
											tmr.reset(0.1);
								...
								new FlxTimer().start(0.2, function(tmr:FlxTimer)
									{
										if (!paused)
										{
											snowstorm.alpha += 0.05; 
											snowstorm2.alpha += 0.05; 
											snowstorm3.alpha += 0.05; 


										}
			
										if (snowstorm.alpha < 0.5)
											tmr.reset(0.1);
									...
									new FlxTimer().start(0.2, function(tmr:FlxTimer)
										{
											if (!paused)
											{
												snowstorm.alpha += 0.04; 
												snowstorm2.alpha += 0.04; 
												snowstorm3.alpha += 0.04; 
											}
				
											if (snowstorm.alpha < 0.4)
												tmr.reset(0.1);
				...
				if (curSong == 'Frostbite')
					{
						switch (curBeat)
						{
							...
							case 18 | 50 | 82 | 114 | 146 | 178 | 210 | 242 | 274 | 306 | 338 | 370 | 402 | 580:
								{
									new FlxTimer().start(0.2, function(tmr:FlxTimer)
										{
											if (!paused)
							...
							case 34 | 66 | 98 | 130 | 162 | 194 | 226 | 258 | 290 | 322 | 354 | 386 | 418:
								{
									new FlxTimer().start(0.2, function(tmr:FlxTimer)
										{
											if (!paused)
			...
			if (curSong == 'Groundhog-Day' && curStage == 'bridge' && !PlayStateChangeables.Optimize) //ughgoddamn
				switch (curBeat)
					{
						case 80:
						{
						...
						/*case 81 | 145 | 241: WELP THIS DIDNT WORK LOL
							{
								remove(bridge);
								remove(bridgecold);
								remove(dad);
			...
			if (curSong == 'Cold-Front' && curStage == 'bridge' && !PlayStateChangeables.Optimize)
				switch (curBeat)
					{
						case 1:
							{
			...
			if (curSong == 'Cryogenic' && curStage == 'bridgecold' && !PlayStateChangeables.Optimize)
				switch (curBeat)
					{
						case 1:
							{
					...
					if (curSong == 'North' && !PlayStateChangeables.Optimize)
						{
							switch (curStep)
							{
					...
					if (curSong == 'Cold-Hearted' && !PlayStateChangeables.Optimize)
						switch (curBeat)
							{
								case 1:
									{
										/*remove(boyfriend);
										boyfriend = new Boyfriend (100, 100, 'bf-playerace'); // playable ace force workaround?
										add(boyfriend);*/
										add(snowfgweak);
										snowfgweak.alpha = 0;
										add(snowfgweak2);

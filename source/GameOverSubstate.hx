...
class GameOverSubstate extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		if (PlayState.deaths >= 2 && !PlayState.shownHint && PlayState.SONG.song != 'Concrete-Jungle' && PlayState.SONG.song != 'Ectospasm' && PlayState.SONG.song != 'Groundhog-Day')
		{
			allowInputs = false;
			allowRetry = false;
		}

		if (FlxG.save.data.specialMechanics && PlayState.SONG.song == 'Ectospasm' && !FlxG.save.data.botplay && !PlayState.loadRep)
			PlayState.deaths++;

		if (PlayState.deaths == 1 && !PlayState.shownHint && PlayState.SONG.song == 'Ectospasm')
		{
			allowInputs = false;
			allowRetry = false;
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (allowInputs && controls.ACCEPT && PlayState.SONG.song != 'Ectospasm')
		{
			if (allowRetry)
				endBullshit();
			else
		...
		if (bf.animation.curAnim.name == 'firstDeath' && bf.animation.curAnim.finished)
		{
			...
			if (PlayState.deaths >= 2 && !PlayState.shownHint && PlayState.SONG.song != 'Ectospasm')
			{
